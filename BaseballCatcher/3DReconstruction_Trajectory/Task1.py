...
print("3D Point in space:")
print(f"Left camera coordinates:\n{point_3d_left.T}")
print(f"Right camera coordinates:\n{point_3d_right.T}")

# Draw circles on the points
draw_circles(left_image_color, points_left, "green")
draw_circles(left_image_color, undistorted_points_left, "red")
draw_circles(right_image_color, points_right, "green")
draw_circles(right_image_color, undistorted_points_right, "red")

# image_with_corners = cv.drawChessboardCorners(left_image_color, (10, 7), points_left, left_ret)
# Display the image with the corners drawn
cv.imshow('Chessboard Corners', left_image_color)
cv.waitKey(0)
cv.imshow('Chessboard Corners', right_image_color)
cv.waitKey(0)
