...
def get_parameters(object_points, image_points, title):
    ...
    parameters = {'ret': ret, 
            ...
            'rvecs': rvecs,
            'tvecs': tvecs}
    np.save(title + '.npy', parameters)
    
    return mtx, dist
            
        
# Load in the data
left_files, left_color_images, left_gray_images = load_data("LeftSet/L")
right_files, right_color_images, right_gray_images = load_data("RightSet/R")
practice_left_files, practice_left_color_images, practice_left_gray_images = load_data("Practice/L")
practice_right_files, practice_right_color_images, practice_right_gray_images = load_data("Practice/R")

# Get points from chess board
left_object_points, left_image_points = get_corners(left_files, left_color_images, left_gray_images)
right_object_points, right_image_points = get_corners(right_files, right_color_images, right_gray_images)
practice_left_object_points, practice_left_image_points = get_corners(practice_left_files, practice_left_color_images, practice_left_gray_images)
practice_right_object_points, practice_right_image_points = get_corners(practice_right_files, practice_right_color_images, practice_right_gray_images)

# Get intrinsic and distortion parameters
left_mtx, left_dist = get_parameters(left_object_points, left_image_points, "Left Camera")
right_mtx, right_dist = get_parameters(right_object_points, right_image_points, "Right Camera")
practice_left_mtx, practice_left_dist = get_parameters(practice_left_object_points, practice_left_image_points, "Practice Left Camera")
practice_right_mtx, practice_right_dist = get_parameters(practice_right_object_points, practice_right_image_points, "Practice Right Camera")