...
ys = ts*m+b
final_estimation = np.round(ts[np.where(ys < 0)[0]][0], 3)

# Plot the time to impact
plt.title(f"Time To Impact Estimation: {final_estimation} frames from the beginning")
...
plt.xlim(0, 50)
plt.ylim(0, 50)
plt.savefig(os.path.join(out_dir, f"time_plot.jpg"))
plt.show()